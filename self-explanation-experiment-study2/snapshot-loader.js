...
window.SESnapshotLoader = (function () {
  ...
  function _validateV3(snapshot) {
    ...
    var ruleRequiredKeys = ["rule_id", "rule_answer", "difficulty", "group", "is_end_requery", "gallery", "test_hands"];
    for (var ri = 0; ri < snapshot.rules.length; ri++) {
      ...
      if (!Array.isArray(rule.test_hands) || rule.test_hands.length !== 6) {
        return FAIL("rules[" + ri + "].test_hands must be an array of exactly 6 entries; got " +
          (Array.isArray(rule.test_hands) ? rule.test_hands.length : typeof rule.test_hands));
      }
      var trueCount = 0;
