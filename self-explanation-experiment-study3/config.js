...
window.SEConfig = (function () {
  ...
  var config = {
    ...
    // Default OFF for v2 / study #2 (feedback would let participants check their
    // hypothesis mid-block, conflating SE with feedback-driven learning).
    // Study #3 (?study3=1) flips the default ON — feedback IS the manipulation.
    // Can still be forced either way with ?feedback=on|off.
    feedback: param("feedback", param("study3", false) ? "on" : "off") === "on",
    ...
    studyMode: param("study2", false) || param("study3", false),  // study3 inherits all study2 behavior

    // study3Mode: set true by ?study3=1. Study 3 = Study 2's exact design PLUS
    // per-trial feedback (default on) PLUS its own data bucket (DataPipe id +
    // results_gallery/study3 + se3_ filename prefix). It rides on top of study2
    // behavior (studyMode above is OR'd), so all study #2 paths are inherited;
    // only save-routing, the filename prefix, and the feedback default differ.
    study3Mode: param("study3", false),

    // splitRange: the per-rule winning/losing split for study #3's variable
    // trial distribution (defeats per-trial-feedback counting). "1-5" (default)
    // draws the winning count from a binomial truncated to 1..5; "2-4" is the
    // tighter fallback. The 6+6 pool snapshot serves both — this only changes
    // the count draw (see exemplar-shuffle.trialCountsFor). Ignored unless study3.
    splitRange: param("splitRange", "1-5"),
    ...
    // isStudy3() — true when ?study3=1. Implies isStudy2() (studyMode is OR'd).
    // Gates only the study #3 data bucket + se3_ filename prefix in app.js.
    isStudy3: function () { return !!this.study3Mode; },

    // splitBounds() — [lo,hi] for the study #3 per-rule winning-count draw.
    splitBounds: function () {
      if (this.splitRange === "1-5") return [1, 5];
      if (this.splitRange === "2-4") return [2, 4];
      throw new Error("SEConfig.splitBounds: invalid splitRange '" + this.splitRange + "'");
    },
