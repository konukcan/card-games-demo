...
window.DevTools = (function () {
  ...
  function skip() {
    // Try, in order: answer the classification trial, advance the study #3
    // self-paced feedback verdict, a Continue screen, or a tutorial Next.
    var winBtn = document.querySelector(".se-btn-win");
    if (winBtn) {
      winBtn.disabled = false;           // enable if gated on recording
      winBtn.click();
      console.log("DevTools: skipped trial (auto-classified as winning)");
      return;
    }
    var verdictNext = document.querySelector(".se-verdict-next");
    if (verdictNext) {
      verdictNext.click();
      console.log("DevTools: advanced past feedback verdict (study #3)");
      return;
    }
