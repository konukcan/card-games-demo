...
window.SEApp = (function () {
  ...
  // DataPipe experiment ID for SE study #3 (feedback) production data. Routes
  // saves to the OSF "data_feedback_version" component (je42q), and the worker
  // fallback to results_gallery/study3 — keeping study #3 data separate from
  // #1 and #2. NOTE: replaces eJqpAuv6LV1y, whose base64/validation config was
  // stuck ON and rejected every save (raw JSON -> INVALID_DATA); this fresh
  // experiment was created with base64+validation OFF and accepts raw JSON
  // (verified: raw POST -> 201, stored as decoded JSON in je42q).
  var DATAPIPE_EXPERIMENT_ID_SE_STUDY3 = "7ViCXn2tUA9Z";
  ...
  async function start() {
    try {
      // Study #3 (feedback): tag <body> so the flip color scheme (win/lose =
      // blue/terracotta, correctness = green/red) and the verdict/record
      // styling scope to study #3 only — study #2 stays sage/coral.
      if (SEConfig.isStudy3()) document.body.classList.add("se-study3");
      ...
      if (SEConfig.isStudy2()) {
        ...
        // Study #3: the per-rule winning-count draw (auditable; sums to 30/30).
        // splitRange records which range fed the binomial. Null in study #2.
        metadata.splitRange = SEConfig.isStudy3() ? SEConfig.splitRange : null;
        metadata.trialCounts = {};
        for (var ci = 0; ci < curriculum.rules.length; ci++) {
          var cr = curriculum.rules[ci];
          if (cr.trialWinningCount != null) {
            metadata.trialCounts[cr.ruleId] = cr.trialWinningCount;
          }
        }
      }
      ...
      if (SEConfig.isStudy2()) {
        ...
        for (var ei = 0; ei < endRequeryIds.length; ei++) {
          ...
          // R2/2.1 (end-requery parity with the v2 bulk loop below): wrap EACH
          // iteration in try/catch so a single failed renderRuleQuery / DOM
          // error does not throw out of the loop and discard the entire session
          // (all classification trials + writeups would never reach the save).
          // On error: record a placeholder, close any open integrity trial,
          // and continue.
          try {
          ...
          endRequeryGuesses.push({
            ...
            console.warn("SEApp: end re-query iteration failed for " +
              erRuleId + ":", erErr);
            // Close any integrity trial left open by the throw so the next
            // iteration starts cleanly.
            try { if (window._integrityMonitor) window._integrityMonitor.endTrial(); } catch (_e) {}
            endRequeryGuesses.push({
              ruleId: erRuleId,
              response: null,
              rt: null,
              error: "endrequery_render_failed: " +
                (erErr && erErr.message ? erErr.message : String(erErr))
            });
          }
        }
      ...
      var filenamePrefix = SEConfig.isStudy3() ? "se3_" : (SEConfig.isStudy2() ? "se2_" : "se_");
      ...
      // Study #3 check FIRST — isStudy3() implies isStudy2() (studyMode is OR'd),
      // so study #3 must claim its own bucket before the study #2 branch.
      if (SEConfig.isStudy3()) {
        saveOptions.workerDir = "results_gallery/study3";
        saveOptions.datapipeExperimentId = DATAPIPE_EXPERIMENT_ID_SE_STUDY3;
      } else if (SEConfig.isStudy2()) {
