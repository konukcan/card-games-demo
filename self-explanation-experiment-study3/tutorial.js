...
window.SETutorial = (function () {
  ...
  async function screenFeedbackExplanation(stepNum, totalSteps) {
    ...
    await showSlide({
      ...
      bodyHTML:
        '<p>After each hand, you will see the answer.</p>' +
        '<p>You will be told whether the hand was <strong>winning</strong> or ' +
        '<strong>losing</strong>, and whether your guess was ' +
        '<span class="se-tutorial-check">Correct</span> or ' +
        '<span class="se-tutorial-cross">Incorrect</span>.</p>' +
        '<p>Take a moment to look at the answer, then press ' +
        '<strong>Next</strong> when you are ready to continue.</p>',
      illustrationFn: function (el) {
        ...
        // Static demo of the verdict (a correct "winning" guess). The Next
        // button is illustrative here — the promise is intentionally not awaited.
        SEUI.showFeedback(focalEl, "winning", true, "winning");
        frame.appendChild(mockDiv);
    ...
    await showSlide({
      ...
      illustrationFn: function (el) {
        ...
        textCol.innerHTML = /* COPY: researcher to confirm */
          ...
          (SEConfig.feedback
            ? '<p>After each guess, we <strong>will</strong> tell you the answer: ' +
              'whether the hand was <strong>winning</strong> or ' +
              '<strong>losing</strong>, and whether your guess was ' +
              '<strong>correct</strong> or <strong>incorrect</strong>. You move on ' +
              'at your own pace by pressing <strong>Next</strong>. The gallery of ' +
              'winning hands also stays on screen the whole time for you to ' +
              'compare, alongside the record of your previous guesses.</p>'
  ...
  async function runPracticeRound() {
    ...
    for (var t = 0; t < rule.testHands.length; t++) {
      ...
      var focalEl = SEUI.renderFocalHand(item.hand, layout.focalArea, {
        selectable: false,
      ...
      // History column: in study #3 (feedback) show the verdict immediately,
      // matching the real round; in study #2 (no feedback) keep it neutral and
      // reveal at the end (Phase 5).
      ...
      // ── Per-trial feedback (study #3 / feedback=on) — self-paced verdict ──
      // Mirror runStudy2Trial exactly: the answer is logged above, then clear
      // the Win/Lose buttons and show the two-line verdict + Next, awaiting the
      // click. Practice rehearses the real feedback rounds, not a batch reveal.
      if (SEConfig.feedback && focalEl) {
        SEUI.setButtonsEnabled(btns, false);
        layout.buttonArea.innerHTML = "";
        await SEUI.showFeedback(focalEl, correctLabel, correct, response);
      }
    ...
    var metaLines = SEConfig.feedback ? [
      "Just like in this practice, in the real rounds you will be told after " +
        "each hand whether you were right.",
