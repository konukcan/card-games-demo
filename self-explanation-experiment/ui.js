...
window.SEUI = (function () {
  ...
  function addToHistoryColumn(histItems, hand, category, response, correct, feedbackOn) {
    ...
    if (feedbackOn) {
      ...
      var overlay = makeEl("div", "se-history-item-overlay", handDiv);
      if (correct) {
  ...
  function renderFocalHand(hand, container, opts) {
    ...
    // Top-of-panel label. Caller can supply opts.label (a pre-built DOM
    // node) to replace the default "Classify This Hand" header — used by
    // the peer condition, where the framing is different (the participant
    // judges another participant's verdict, not the hand directly).
    if (opts.label instanceof Node) {
      panel.appendChild(opts.label);
    } else {
      var label = makeEl("div", "se-focal-panel-label", panel);
      label.textContent = "Classify This Hand";
    }
  ...
  function showFeedback(focalEl, category, correct) {
    focalEl.classList.add("se-feedback");

    if (category === "winning") {
      focalEl.classList.add("fb-winning");
    } else {
      focalEl.classList.add("fb-losing");
    }

    var overlay = makeEl("div", "se-feedback-overlay", focalEl);
    if (correct) {
      overlay.classList.add("correct");
      overlay.textContent = "\u2713";
    } else {
  ...
  function renderTutorialStep(config) {
    ...
    var nextBtn = makeEl("button", "se-tutorial-btn se-tutorial-btn-primary", btnGroup);
